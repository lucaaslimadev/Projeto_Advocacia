...
const SessionsModal = ({ sessions, onClose, onUpdate }) => {
  ...
  const handleCreateSession = async () => {
    ...
    try {
      await ipcRenderer.invoke('create-sessao', newSessionName.trim());
      setNewSessionName('');
  ...
  const handleDeleteSession = async (sessionId, sessionName) => {
    if (window.confirm(`Tem certeza que deseja excluir a sessão "${sessionName}"?`)) {
      try {
        await ipcRenderer.invoke('delete-sessao', sessionId);
        onUpdate(); // Atualiza a lista sem fechar o modal
      } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gray-900 rounded-xl shadow-2xl w-full max-w-lg mx-4">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-700">
          <h2 className="text-xl font-semibold text-white">Gerenciar Sessões</h2>
          <button
            onClick={onClose}
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {sessions.map((session) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() => handleDeleteSession(session.id, session.nome)}
